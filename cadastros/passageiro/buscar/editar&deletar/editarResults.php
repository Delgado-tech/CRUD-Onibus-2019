...
<?php
	...
	echo '
	<form name="searchUserForm" method="POST" action="editarResults.php">

	<nav>
    <div class="nav-wrapper orange darken-4">
      <form>
        <div class="input-field">
          <input id="search" placeholder="Pesquisar Usuário..." name="navbar" value="'.$navbar.'" type="search" required>
  ...
  <a class='btn tooltipped orange darken-4' href='../../cadPassMain.php'><b><</b></a>
      
  ";
	
	$sql = mysqli_query($conexao,"SELECT * FROM CAD_PASSAGEIRO WHERE CPF LIKE '%$navbar%' OR PASS_NAME LIKE '%$navbar%' ORDER BY PASS_NAME ASC");
	
	$row = mysqli_num_rows($sql);
	
	if($row > 0){
	echo '<h4>Resultado da pesquisa: "'.$navbar.'"</h4>';
	...
	echo "
        <tbody>
          <tr>
            <td style='color: gray;'>$cpf</td>
            <td style='color: gray;'>$name</td>
            <td style='color: gray;'>$cidade</td>
			<td style='color: gray;'>$estado</td>
			<td style='color: gray;'>$nasc</td>
			<td style='color: gray;'>";
				
		if($obs == "---"){
		echo "<p style='margin-left:35px; color: gray;'><b>---</b></p>";
		}else{include '../../../obs.php';}
				
			echo "</td>
          
		  <td><a href='alterarPass.php?
			cpf=$cpf&&
			name=$name&&
			nasc=$nasc&&
			cidade=$cidade&&
			estado=$estado&& 
			obs=$obs
			' class='btn-floating pulse blue'><i class='small material-icons'>edit</i></a></td>
			<td style='color: darkred'>
			
			<a href='deletarPass.php?cpf=".$cpf."' 
			class='btn-floating pulse #b71c1c red darken-4'><i class='small material-icons'>delete</i></a></td>
		...
		echo "<center><h1>Nenhum Usuário Encontrado</h1></center>";
	}
