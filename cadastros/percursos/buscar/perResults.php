...
$navbar = $_POST['navbar'];
    ...
    <div class="nav-wrapper grey darken-3">
      <form>
        <div class="input-field">
          <input value="'.$navbar.'" id="search" placeholder="Pesquisar Percurso..." name="navbar" type="search" required>
  ...
  <a class='btn tooltipped grey darken-3' href='../cadPerMain.php'><b><</b></a>
      
  ";
	
	$sql = mysqli_query($conexao,"SELECT * FROM CAD_PERCURSO 
	WHERE ROTA LIKE '%$navbar%' 
	ORDER BY ROTA ASC");
	...
	echo '<h4>Resultado da pesquisa: "'.$navbar.'"</h4>';
		...
		while($linha = mysqli_fetch_array($sql))
		{
			...
			$PASS4 = "R$ ".$PASS_VALOR0.".".$PASS_VALOR1;
			...
			$PASSB4 = "R$ ".$BAG_VALOR0.".".$BAG_VALOR1;
	...
	echo "
        <tbody>
          <tr>
			...
			<td><a href='perPDF.php?rota=$ROTA&&
			pertemp=$TEMP_AROUND&&
			passvalor=$PASS4&&
			passbag=$PASSB4&&
			qtdemot=$QTDE_MOT&&
			trocarMot=$TROCAR_MOT&&
			openvenda=$OPEN_VENDAS' class='btn-floating grey darken-3' style='margin-left: 10px'><i class='small material-icons'>assignment</i></a></td>
		...
		echo "<center><h1>Nenhum Percurso Encontrado</h1></center>";
	}
