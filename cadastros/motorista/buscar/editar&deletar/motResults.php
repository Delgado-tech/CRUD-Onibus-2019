...
$navbar = $_POST['navbar'];
    ...
    <div class="nav-wrapper green darken-4">
      <form>
        <div class="input-field">
          <input value="'.$navbar.'" id="search" placeholder="Pesquisar Motorista..." name="navbar" type="search" required>
  ...
  <a class='btn tooltipped green darken-4' href='../../cadMotMain.php'><b><</b></a>
      
  ";
	
	$sql = mysqli_query($conexao,"SELECT * FROM CAD_MOTORISTA 
	WHERE COD_MOT LIKE '%$navbar%' OR MOT_NAME LIKE '%$navbar%' OR CNH_TIPO LIKE '%$navbar%'
	ORDER BY MOT_NAME ASC");
	
	$row = mysqli_num_rows($sql);
	
	if($row > 0){
	echo '<h2>Resultado da pesquisa: "'.$navbar.'"</h2>';
        ...
        <thead>
          <tr>
			  ...
			  <th>Venc. CNH</th>
			  <th>Nascimento</th>
			  <th><p style='margin-left: 10px'>Endereço</p></th>
	...
	echo "
        <tbody>
          <tr>
            <td style='color: grey'><b>#$Id</b></td>
            <td style='color: grey'>$name</td>
			<td style='color: grey'>$fone</td>
            <td style='color: grey'><p style='margin-left: 50px'>$cnhtipo</p></td>
			<td style='color: grey'>$cnhvenc</td>
			<td style='color: grey'>$nasc</td>
			<td>";
				
		include '../../../obs.php';
				
			echo "</td>
			<td><a href='alterarMot.php?Id=$Id&& 
			name=$name&&
			nasc=$nasc&&
			cnhvenc=$cnhvenc&&
			cnhtipo=$cnhtipo&&
			...
			bairro=$bairro&&
			numcasa=$numcasa
			' class='btn-floating pulse blue' style='margin-left: 10px'><i class='small material-icons'>edit</i></a></td>
			<td style='color: darkred'>
			
			<a href='deletarMot.php?codmot=".$Id."' 
		...
		echo "<center><h1>Nenhum Motorista Encontrado</h1></center>";
	}
