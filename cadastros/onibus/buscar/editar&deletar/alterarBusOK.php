...
<head>
	...
	$placa != $placafinal || strlen($obs) > 1000 || strlen($anofab) != 4 || strlen($assentos) > 2) //filtro de erro
	{
		...
		}else if($placasize != 7 || $placa != $placafinal){
			echo '
		...
		$sql = mysqli_query($conexao,"UPDATE CAD_ONIBUS SET PLACA='$placa2',MARCA='$marca',
		TIPO_AUTO='$bustype',FAB_ANO='$anofab',NUM_ASSENTO='$assentos',OBS_AUTO='$obs'
		WHERE PLACA = '$placa1'") or die(mysqli_error($conexao));
		
		$sql = mysqli_query($conexao,"UPDATE LCMT_VIAGEM SET PLACA2='$placa2'
		WHERE PLACA2 = '$placa1'") or die(mysqli_error($conexao));
		
		echo "<div class='container'>
		<h1 class='center-align'>Atualizando Ônibus! Aguarde...</h1>
		";
